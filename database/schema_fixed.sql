-- =====================================================
-- سیستم انبارداری دارو - اسکریپت دیتابیس (اصلاح شده)
-- طراحی و توسعه: علیرضا حامد - پاییز 1404
-- =====================================================

-- پاک کردن جداول موجود (اختیاری - فقط در صورت نیاز)
...
-- اضافه کردن ستون active اگر وجود ندارد
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='warehouses' AND column_name='active') THEN
        ALTER TABLE public.warehouses ADD COLUMN active BOOLEAN DEFAULT true;
    END IF;
END $$;
...
-- اضافه کردن ستون‌های مفقود اگر وجود ندارند
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='drugs' AND column_name='active') THEN
        ALTER TABLE public.drugs ADD COLUMN active BOOLEAN DEFAULT true;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='drugs' AND column_name='features') THEN
        ALTER TABLE public.drugs ADD COLUMN features TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='drugs' AND column_name='image_url') THEN
        ALTER TABLE public.drugs ADD COLUMN image_url VARCHAR(500);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='drugs' AND column_name='barcode') THEN
        ALTER TABLE public.drugs ADD COLUMN barcode VARCHAR(100);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='drugs' AND column_name='min_stock_level') THEN
        ALTER TABLE public.drugs ADD COLUMN min_stock_level INTEGER DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='drugs' AND column_name='max_stock_level') THEN
        ALTER TABLE public.drugs ADD COLUMN max_stock_level INTEGER DEFAULT 1000;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='drugs' AND column_name='unit_price') THEN
        ALTER TABLE public.drugs ADD COLUMN unit_price DECIMAL(10,2) DEFAULT 0;
    END IF;
END $$;
...
-- اضافه کردن UNIQUE constraint اگر وجود ندارد
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='warehouse_inventory_warehouse_id_drug_id_batch_number_key') THEN
        ALTER TABLE public.warehouse_inventory ADD CONSTRAINT warehouse_inventory_warehouse_id_drug_id_batch_number_key UNIQUE(warehouse_id, drug_id, batch_number);
    END IF;
END $$;
...
-- =====================================================
-- اضافه کردن Foreign Key برای users.warehouse_id
-- =====================================================
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='fk_users_warehouse_id') THEN
        ALTER TABLE public.users ADD CONSTRAINT fk_users_warehouse_id 
...
CREATE INDEX IF NOT EXISTS idx_inventory_warehouse_id ON public.warehouse_inventory(warehouse_id);
...
CREATE INDEX IF NOT EXISTS idx_movements_type ON public.drug_movements(movement_type);
CREATE INDEX IF NOT EXISTS idx_movements_date ON public.drug_movements(movement_date);
CREATE INDEX IF NOT EXISTS idx_movements_reference ON public.drug_movements(reference_number);
