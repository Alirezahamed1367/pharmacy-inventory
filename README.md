...
- جستجو و فیلتر پیشرفته
 - امکان فعال / غیرفعال‌سازی دارو (فیلد `is_active`) به جای حذف برای حفظ تاریخچه
 - Snapshot نام و تاریخ انقضا داخل آیتم‌های رسید و حواله برای مصونیت از تغییرات آینده
...
6. پیش از اعمال Migration فاز 2 (NOT NULL lots):
   ...
   npm run audit:lots
   ```
7. پاکسازی موجودی صفر (اختیاری):
   ```bash
   node scripts/cleanupZeroInventory.mjs
   ```
...
### نکات جدید مدل داده
- فیلد `drugs.is_active` برای غیرفعال‌سازی منطقی.
- ستون‌های snapshot: `receipt_items.(drug_name_snapshot, expire_date_snapshot)` و `transfer_items.(drug_name_snapshot, expire_date_snapshot)`.
- اسکریپت تمیزکاری: `scripts/cleanupZeroInventory.mjs`.
